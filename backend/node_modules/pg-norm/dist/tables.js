...
export class PG_Table {
    ...
    async insert(data) {
        ...
        for (const key of keys) {
            if (!this.visibles.includes(key)) {
                throw new Error("inserted rows need to have all columns in visibles (unexpected column " + key + ")");
            }
        }
    ...
    async update(row_id, data) {
        ...
        for (const key of keys) {
            if (!this.visibles.includes(key)) {
                throw new Error("inserted rows need to have all columns in visibles (unexpected key: " + key + ")" + JSON.stringify(this.visibles));
            }
        }
...
export class PG_AuthTable extends PG_Table {
    ...
    table_name;
    identify_user_by;
    dummy_password_hash = "$2a$12$4jMZgsZF8HpkBKETdDSKDOIuFwkwYTppUbap/RbTyRCpFuHa2UoCe";
     ...
     */
    async idAfterAuth(user_identifier, plainTextPassword) {
        const [user] = await this.sql`
     ...
     */
    async fetchAfterAuth(user_identifier, plainTextPassword, columns) {
        for (const column of columns) {
            if (!this.visibles.includes(column))
                throw new ReferenceError("access to non-viisble non-authorized columns is restricted");
        }
        const [user] = await this.sql`
