...
export async function markStudentAbsent(metadata, data) {
...
export async function fetchAbsentStudents(metadata, attendance_record_id) {
    const [record] = await attendance_record.fetch(attendance_record_id);
    if (!record) {
        throw new Error("Attendance record not found");
    }
