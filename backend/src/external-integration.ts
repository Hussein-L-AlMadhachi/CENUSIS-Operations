import { createRPC, type RPCHandler } from "enders-sync";
...
export function createBotRPC(app: Express, ...methods_list: RPCHandler[]) {
    let bots_configs: any[] = [];
    try {
    ...
    for (const bot_config of bots_configs) {
        ...
        if ((auth as any)[bot_config.role] !== undefined) {
