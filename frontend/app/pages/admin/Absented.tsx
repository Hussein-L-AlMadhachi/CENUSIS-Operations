...
// Globals
import { type AbsentedData, adminRPC } from "@/rpc";
...
function AddAbsentStudentModal({ isOpen, onClose, onSuccess, record_id }: AddAbsentStudentModalProps) {
    const handleAddAbsentStudent = async (data: any) => {
        ...
        try {
            await adminRPC.markStudentAbsent({
    ...
    return (
        <Modal isOpen={isOpen} className="w-full flex flex-col justify-center max-w-lg">
            ...
            <DynamicForm
                ...
                template={[
                    { title: "اسم الطالب", key: "student_name", type: "autocomplete", fetchSuggestions: async (query: string) => await adminRPC.autocompleteStudent(query) },
...
function MainContent(): JSX.Element {
    ...
    const fetchData = () => {
        adminRPC.fetchAbsentStudents(attendance_record_id).then((data) => setData(data));
    };
        ...
        "@delete": (row: AbsentedData) => {
            ...
            return (
                <div className="flex flex-col flex-nowrap">
                    <button onClick={() => adminRPC.removeAbsence(row.id!).then(() => navigate("/admin/subjects/"))} className="btn btn-xs">
                        حذف
                    </button>
                </div>
            )
...
export function AbsentedPage(): JSX.Element {
