...
function AddThresholdModal({ isOpen, onClose, onSuccess, selectedGradingSystem }: AddThresholdModalProps) {
    const handleSubmit = async (data: AbsenceAlertThresholdData) => {
        ...
        await superAdminRPC.newAbsenceAlertThreshold({
...
function MainContent(): JSX.Element {
    ...
    const refreshAlerts = useCallback(async () => {
        ...
        try {
            await superAdminRPC.recomputeAbsenceAlerts();
            const [
                thresholdRows,
                firstClassRows,
                secondClassRows,
                ...
                superAdminRPC.fetchAbsenceAlertThresholds({ grading_system_name: selectedGradingSystem }),
                superAdminRPC.fetchAbsenceAlerts({ degree: "بكلوريوس", class: 1, grading_system_name: selectedGradingSystem }),
                superAdminRPC.fetchAbsenceAlerts({ degree: "بكلوريوس", class: 2, grading_system_name: selectedGradingSystem }),
                superAdminRPC.fetchAbsenceAlerts({ degree: "بكلوريوس", class: 3, grading_system_name: selectedGradingSystem }),
                superAdminRPC.fetchAbsenceAlerts({ degree: "بكلوريوس", class: 4, grading_system_name: selectedGradingSystem }),
                superAdminRPC.fetchAbsenceAlerts({ degree: "ماجستير", grading_system_name: selectedGradingSystem }),
                superAdminRPC.fetchAbsenceAlerts({ degree: "دكتوراه", grading_system_name: selectedGradingSystem })
            ]);
    ...
    useEffect(() => {
        superAdminRPC.fetchGradingSystems().then((systems) => {
        ...
        <Section>
            ...
            <Subsection>
                ...
                <EditableTable
                    ...
                    onSave={async (id, row) => {
                        await superAdminRPC.updateAbsenceAlertThreshold(id, {
                            grading_system_name: selectedGradingSystem,
                    ...
                    onDelete={async (id) => {
                        await superAdminRPC.deleteAbsenceAlertThreshold(id);
                        await refreshAlerts();
                    }}
...
export function SuperAbsenceAlertsPage(): JSX.Element {
